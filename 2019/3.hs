...
sol1' :: (Int,Int) -> Set.Set (Int,Int) -> [(Char,Int)] -> Set.Set (Int,Int)
sol1' _   s [] = s
sol1' pos s ((d,0):xs) = sol1' pos s xs -- (Set.insert pos s) xs
sol1' pos@(x,y) s ((d,n):xs) = sol1' pos' (Set.insert pos' s) ((d,n-1):xs)
  where pos' = case d of
                 'R' -> (x+1,y)
                 'D' -> (x,y+1)
                 'L' -> (x-1,y)
...
sol1 xs = manhattan (0,0) $ minimumBy (comparing $ manhattan (0,0)) (Set.toList c)
  where [a,b] = map (sol1' (0,0) Set.empty) xs
        c = a `Set.intersection` b


sol2' :: (Int,Int) -> Int -> Map.Map (Int,Int) Int -> [(Char,Int)] -> Map.Map (Int,Int) Int
sol2' _ _ s [] = s
sol2' pos k s ((d,0):xs) = sol2' pos k s xs
sol2' pos@(x,y) k s ((d,n):xs) = sol2' pos' (k+1) (Map.insert pos' (k+1) s) ((d,n-1):xs)
  where pos' = case d of
                 'R' -> (x+1,y)
                 'D' -> (x,y+1)
                 'L' -> (x-1,y)
...
sol2 xs = minimum [ k1+k2 | (p1,k1) <- Map.toList c, let Just k2 = Map.lookup p1 b ]
  where [a,b] = map (sol2' (0,0) 0 Map.empty) xs
        c = a `Map.intersection` b

main = do let i = [ map parse xs | xs <- map (splitOn ',') input1 ]
          print (sol1 i)
          print (sol2 i)
